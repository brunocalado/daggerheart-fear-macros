...
export class StressMacroConfig extends foundry.applications.api.HandlebarsApplicationMixin(foundry.applications.api.ApplicationV2) {
    ...
    static DEFAULT_OPTIONS = {
        ...
        tag: "div",
        window: {
            title: "Daggerheart: Stress Macro Configuration",
            resizable: false
        },
        position: { width: 480 }
    };
    ...
    static PARTS = {
        form: { template: `modules/${MODULE_ID}/templates/stress-macro-config.hbs` }
    };
    ...
    async _prepareContext(options) {
        const slots = [
            { key: "stressIncrease", label: "Increase Stress", hint: "Triggered when a player actor's Stress increases (not at Max)." },
            { key: "stressDecrease", label: "Decrease Stress", hint: "Triggered when a player actor's Stress decreases (not 0)." },
            { key: "stressMax",      label: "Stress Max",      hint: "Triggered instead of Increase when Stress reaches its maximum value." },
            { key: "stressZero",     label: "Stress Zero (0)", hint: "Triggered instead of Decrease when Stress reaches exactly 0." }
        ];
